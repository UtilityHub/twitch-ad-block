...
(function() {
    ...
    function declareOptions(scope) {
        ...
        // Stealth: random class for ad banner
        if (scope.OPT_STEALTH_MODE) {
            scope.UBO_OVERLAY_CLASS = 'ubo-' + Math.random().toString(36).substring(2, 10);
        } else {
            scope.UBO_OVERLAY_CLASS = 'ubo-overlay';
        }
    }
    var twitchWorkers = [];
        ...
        var newWorker = class Worker extends getCleanWorker(window.Worker) {
            constructor(twitchBlobUrl, options) {
                ...
                function getAdDiv() {
                    ...
                    var adDiv = null;
                    var overlayClass = typeof UBO_OVERLAY_CLASS !== 'undefined' ? UBO_OVERLAY_CLASS : 'ubo-overlay';
                    if (playerRootDiv != null) {
                        adDiv = playerRootDiv.querySelector('.' + overlayClass);
                        if (adDiv == null) {
                            adDiv = document.createElement('div');
                            adDiv.className = overlayClass;
    ...
    function onFoundAd(streamInfo, textStr, reloadPlayer) {
        if (!(typeof OPT_STEALTH_MODE !== 'undefined' && OPT_STEALTH_MODE)) {
